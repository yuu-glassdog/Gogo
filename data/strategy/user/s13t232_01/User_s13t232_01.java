package data.strategy.user.s13t232_01;
...
public class User_s13t232_01 extends GogoCompSub {
  
//====================================================================
//  コンストラクタ
//====================================================================

  public User_s13t232_01(GamePlayer player) {
    super(player);
    name = "s13t232";    // プログラマが入力
  }
  ...
  public void calc_values(GameState prev, GameBoard board) {
    ...
    for (int i = size-1; i >= 0; i--) {
      for (int j = 0; j < size; j++) {
        ...
        // 三々の禁じ手は打たない → -1
        //if ( check_three(cell, mycolor, i, j, 3) ) { values[i][j] = -1; }
        // 相手の石を取る → 300;
        if ( check_rem(cell, mycolor, i, j) ) { values[i][j] = 450; }
        // 自分の石を守る → 200;
...
//----------------------------------------------------------------
  ...
  boolean check_three(int[][] board, int color, int i, int j, int len) {
    ...
    for ( int dx = -1; dx <= 1; dx++ ) {
      for ( int dy = -1; dy <= 1; dy++ ) {
        if ( dx == 0 && dy == 0 ) { continue; }
        if ( check_run_dir(board, color, i, j, dx, dy, len) ) { ct++; }
      }
    }
